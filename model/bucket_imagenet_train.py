...
if __name__ == '__main__':
    ...
    assert set(val_buckets)==set(val_buckets)
    assert len(set(val_buckets))==len(val_buckets)
    
    n_buckets = len(train_buckets)
    
    models = {}
    for bucket in train_buckets:   
        print('training model from data in '+bucket)
        ...
        train_set = modules.ImageNet(root='data/bucket_imagenet/train', file=bucket, transform=transform_ops)
        test_set = modules.ImageNet(root='data/bucket_imagenet/valid', file=bucket, transform=transform_ops)
